  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      tap(() => {
        ...
        if (!existingHeaders['x-ratelimit-limit'] && !existingHeaders['x-ratelimit-remaining']) {
        ...
        response.setHeader('X-RateLimit-Policy-Info', 'Public: 100/min per IP, Auth: 1000/min per user, Search: 30/min per IP');
      }),
    );
  }
}